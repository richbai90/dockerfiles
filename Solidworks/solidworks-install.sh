...
if VERB="$( which apt-get )" 2> /dev/null; then
   echo "Debian-based"
    apt-get update &&
    apt-get install dialog wmctrl software-properties-common
elif VERB="$( which dnf )" 2> /dev/null; then
   echo "RedHat-based"
    dnf update &&
    dnf install dialog wmctrl
elif VERB="$( which pacman )" 2> /dev/null; then
   echo "Arch-based"
    pacman -Syu --needed dialog wmctrl
   ...
   echo "Void-based"
    xbps-install -Sy dialog wmctrl
elif VERB="$( which eopkg )" 2> /dev/null; then
   echo "Solus-based"
    eopkg install dialog wmctrl
elif VERB="$( which emerge )" 2> /dev/null; then
    echo "Gentoo-based"
     emerge -av dev-util/dialog x11-misc/wmctrl
else
...
case $CHOICE in         
        ...
        11) 
            ...
            debian-based-1 &&
             apt-add-repository -r 'deb https://dl.winehq.org/wine-builds/ubuntu/ bionic main' &&
            wget -q https://download.opensuse.org/repositories/Emulators:/Wine:/Debian/xUbuntu_18.04/Release.key -O Release.key -O- |  apt-key add - &&
        ...
        12) 
            ...
            debian-based-1 &&
             add-apt-repository -r 'deb https://dl.winehq.org/wine-builds/ubuntu/ focal main' &&
            wget -q https://download.opensuse.org/repositories/Emulators:/Wine:/Debian/xUbuntu_20.04/Release.key -O Release.key -O- |  apt-key add - &&
        ...
        13) 
            ...
            debian-based-1 &&
             add-apt-repository -r 'deb https://dl.winehq.org/wine-builds/ubuntu/ groovy main' &&
            wget -q https://download.opensuse.org/repositories/Emulators:/Wine:/Debian/xUbuntu_20.10/Release.key -O Release.key -O- |  apt-key add - &&
             apt-add-repository 'deb https://download.opensuse.org/repositories/Emulators:/Wine:/Debian/xUbuntu_20.10/ ./' &&
            debian-based-2
            ;;
        ...
        14)
            ...
            gpg --no-default-keyring --keyring ./temp-keyring.gpg --export --output winehq.gpg && rm temp-keyring.gpg &&
             mv *.gpg /etc/apt/trusted.gpg.d/ && cd /tmp &&  rm -rf 360 &&
            echo "deb [signed-by=/etc/apt/trusted.gpg.d/opensuse-wine.gpg] https://download.opensuse.org/repositories/Emulators:/Wine:/Debian/xUbuntu_21.04/ ./" |  tee -a /etc/apt/sources.list.d/opensuse-wine.list
             add-apt-repository -r 'deb https://dl.winehq.org/wine-builds/ubuntu/ hirsute main' &&
            debian-based-2
            ;;
...
function debian-based-2 {
     apt-get update &&
     apt-get upgrade &&
     apt-get install p7zip p7zip-full p7zip-rar curl winbind cabextract wget &&
...
function fedora-based-2 {
     dnf install p7zip p7zip-plugins curl wget wine cabextract &&
    select-your-path
}
...
function solus-linux {
    eopkg install wine winetricks p7zip curl cabextract samba ppp
}

function void-linux {
    xbps-install -Sy wine wine-mono wine-gecko winetricks p7zip curl cabextract samba ppp
}

function gentoo-linux {
     emerge -av virtual/wine app-emulation/winetricks app-emulation/wine-mono app-emulation/wine-gecko app-arch/p7zip app-arch/cabextract net-misc/curl net-fs/samba net-dialup/ppp
}
